...
class TritonKernel(SIMDKernel):
    ...
    def reduction(
        ...
        original_dtypes = [v.dtype for v in value] if isinstance(value, tuple) else [value.dtype]
        ...
        if isinstance(result_var, tuple):
            ...
            for var, _dtype in zip(result_var, original_dtypes):
                casted_result_var.append(
                    ops.to_dtype(result_var, _dtype)
                    if var.dtype != _dtype
                    else var
                )
            result_var = tuple(casted_result_var)
        else:
            assert isinstance(result_var, TritonCSEVariable)
            self.outside_loop_vars.add(result_var)
            ...
            # Match output dtype with input dtype
            result_var = (
                ops.to_dtype(result_var, original_dtypes[0])
